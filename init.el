...
(add-hook 'rust-mode-hook
	  (lambda ()
	    (local-set-key (kbd "C-G") #'cargo-process-run)
	    (add-to-list 'compilation-error-regexp-alist '("^.*?\\(\w.*\\):\\([0-9]+\\):\\([0-9]+\\).*$" 1 2 3))))
